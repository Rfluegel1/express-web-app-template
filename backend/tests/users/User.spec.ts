...
describe('User object', () => {
	...
	it('values constructor sets values', () => {
		// when
		const result: User = new User('the email', 'the passwordHash', true, 'emailVerificationToken',  { token: 'token', expiration: 'expiration' }, 'role', 'passwordResetToken', 'emailUpdateToken', 'pendingEmail');
		// then
		expect(result.id).toMatch(UUID_REG_EXP);
		...
		expect(result.emailVerification).toEqual({ token: 'token', expiration: 'expiration' });
